...
while True:
    ...
    if (status == ProtocolStatus.MSG_OK):
        ...
        if (dp.cmd == 2):
            ...
            try:
                c.execute("INSERT INTO things_thing VALUES (?,?,?,?,?,?)",[r.name[:7], r.src, r.type, 0, dt, 0])
                print "Added dev in db %s" % r.name[:7]
            except:
                c.execute("UPDATE things_thing SET addr=?, dev_type=?, refreshed=? WHERE name=?", [r.src, r.type, dt, r.name[:7]])
